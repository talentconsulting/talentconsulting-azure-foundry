"""Responses-protocol host for the local dev config workflow."""
...
DISCOVERY_NAME = os.getenv("LOCAL_DEV_CONFIG_DISCOVERY_AGENT_NAME", "local-dev-config-source-discovery")
GENERATOR_NAME = os.getenv("LOCAL_DEV_CONFIG_GENERATOR_AGENT_NAME", "local-dev-config-generator")
...
def error_response(error: Exception, source_url: str = "") -> dict[str, Any]:
    ...
    message = str(error) if isinstance(error, WorkflowError) else "The local dev config workflow could not be completed."
    return {
        "success": False,
