...
def make_catalog(repository="source/app", ref="main", path="src", projects=None, sdks=None):
    return {
...
def manifest(last_commit=OLD_SHA, extra_entries=None):
    entries = [{
        ...
        "dotnet-version": {"path-to-scan": "tree/main/src", "last-commit-hash-scanned": last_commit},
    }]
    if extra_entries:
...
class ManifestTests(unittest.TestCase):
    def test_validate_manifest_skips_entries_without_dotnet_version_node(self):
        shared = manifest("")
        shared.append({
    ...
    def test_validate_manifest_rejects_duplicate_repositories(self):
        ...
        duplicated.append({
            "github-repo": "https://github.com/source/app",
            "dotnet-version": {"path-to-scan": "tree/main/other", "last-commit-hash-scanned": ""},
        })
        with self.assertRaises(ManifestError):
    ...
    def test_run_manifest_updates_commit_hash_only_on_success(self):
        two_repo_manifest = manifest(OLD_SHA, extra_entries=[{
            "github-repo": "https://github.com/source/other",
            "dotnet-version": {"path-to-scan": "tree/main/src", "last-commit-hash-scanned": OLD_SHA},
        }])
