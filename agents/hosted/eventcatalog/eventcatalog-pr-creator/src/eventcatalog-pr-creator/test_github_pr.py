...
CATALOG = {"repository": "source/app", "ref": "main", "path": "src", "commands": [], "events": []}
...
def payload():
    return {
        ...
        "catalogs": [{
            ...
            "catalog": CATALOG,
            "targetPath": "app/event-catalog/events-and-commands.json",
        }],
    }
...
class PublisherTests(unittest.TestCase):
    def test_rejects_invalid_catalog(self):
        request = payload()
        request["catalogs"][0]["catalog"] = {"commands": []}
    ...
    def test_creates_one_commit_and_pull_request(self):
        client = FakeClient()
        result = publish(payload(), client, branch_factory=lambda: "eventcatalog/test")
        self.assertEqual("created", result["status"])
        self.assertEqual("app/event-catalog/events-and-commands.json", result["filesWritten"][0]["path"])
    ...
    def test_catalog_and_manifest_share_one_tree(self):
        ...
        client = FakeClient()
        result = publish(request, client, branch_factory=lambda: "eventcatalog/manifest")
