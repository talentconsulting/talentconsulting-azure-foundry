"""Responses-protocol host for the dotnet-version PR creator."""
...
def error_response(error: Exception) -> dict[str, object]:
    ...
    message = str(error) if isinstance(error, PublicationError) else "The .NET version catalog could not be published."
    return {
...
async def handle_create(request: CreateResponse, context: ResponseContext, cancellation_signal: asyncio.Event):
    ...
    try:
        ...
        logger.exception(".NET version publication failed.")
