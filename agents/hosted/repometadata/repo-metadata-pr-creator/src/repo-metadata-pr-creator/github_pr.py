...
class GitHubClient:
    ...
    def request(self, method: str, path: str, payload: object | None = None) -> Any:
        ...
        headers = {
            ...
            "User-Agent": "repo-metadata-pr-creator",
...
def validate_request(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    target_directory = _clean_relative_path(payload.get("targetDirectory", "repo-metadata"), "targetDirectory")
    validated = []
    ...
    for item in catalogs:
        ...
        if not isinstance(catalog, dict) or set(catalog) != {
            ...
            raise PublicationError(
                "invalid_catalog",
                "Each catalog must contain repository, ref, path, lastCommitDate, projects, sdks, and dotnetSupport.",
            )
        if (
            not isinstance(catalog["repository"], str)
            or not isinstance(catalog["ref"], str)
            or not isinstance(catalog["path"], str)
            or not isinstance(catalog["lastCommitDate"], str)
        ...
        output_path = _clean_relative_path(
            item.get(
                "targetPath",
                posixpath.join(_source_repository(item["sourceUrl"]), target_directory, "repo-metadata.json"),
            ),
            "targetPath",
        )
    ...
    if total_bytes > MAX_TOTAL_BYTES:
        raise PublicationError("catalogs_too_large", "Generated repo-metadata files exceed 10 MiB.")
...
def default_branch_name() -> str:
    ...
    return f"repo-metadata/{timestamp}-{uuid.uuid4().hex[:8]}"
...
def publish(payload: dict[str, Any], client: GitHubClient | None = None, branch_factory: Callable[[], str] = default_branch_name) -> dict[str, Any]:
    ...
    pull_request = github.request("POST", f"/repos/{repository}/pulls", {
        ...
        "head": branch_name,
        "base": base_branch,
        "body": request.get("pullRequestBody", "Generated by the repo-metadata workflow."),
    })
    return {
