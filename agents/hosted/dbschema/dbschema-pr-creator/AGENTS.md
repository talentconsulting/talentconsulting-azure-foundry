...
- Keep runtime code under `src/dbschema-pr-creator/`.
- Never commit GitHub credentials; read `GITHUB_PR_TOKEN` from the environment.
- Commit every supplied schema and optional manifest update together, then create at most one branch and pull request.
