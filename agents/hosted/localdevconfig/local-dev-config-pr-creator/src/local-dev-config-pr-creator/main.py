"""Responses-protocol host for the local-dev-config PR creator."""
...
def error_response(error: Exception) -> dict[str, object]:
    ...
    message = str(error) if isinstance(error, PublicationError) else "The local development configuration could not be published."
    return {
...
async def handle_create(request: CreateResponse, context: ResponseContext, cancellation_signal: asyncio.Event):
    ...
    try:
        ...
        logger.exception("Local-dev-config publication failed.")
