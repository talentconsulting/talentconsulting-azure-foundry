"""Responses-protocol host for the system-summary PR creator."""
...
def error_response(error: Exception) -> dict[str, object]:
    ...
    message = str(error) if isinstance(error, PublicationError) else "The system summaries could not be published."
    return {
...
async def handle_create(request: CreateResponse, context: ResponseContext, cancellation_signal: asyncio.Event):
    ...
    try:
        ...
        logger.exception("System-summary publication failed.")
