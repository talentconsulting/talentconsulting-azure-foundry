...
def validate_openapi_output(payload: dict[str, Any]) -> None:
    required_output = {
        "success", "sourceDirectoryUrl", "apiFiles", "specs", "errors"
    }
    if set(payload) != required_output:
        raise ValueError(
            "OpenAPI workflow response does not match its output contract."
        )
    if payload["success"] is not True:
        raise ValueError("OpenAPI workflow did not complete successfully.")
    if not isinstance(payload["sourceDirectoryUrl"], str):
        raise ValueError("OpenAPI workflow sourceDirectoryUrl must be a string.")
    if not isinstance(payload["errors"], list) or payload["errors"]:
        raise ValueError("OpenAPI workflow returned generation errors.")
    api_files = payload["apiFiles"]
    if not isinstance(api_files, list):
        raise ValueError("OpenAPI workflow apiFiles must be an array.")
    scanned_files: list[str] = []
    for index, item in enumerate(api_files):
        if not isinstance(item, dict) or set(item) != {"apiFilePath", "payloadFiles"}:
            raise ValueError(f"apiFiles[{index}] does not match the scan contract.")
        path = item["apiFilePath"]
        if not isinstance(path, str) or not path or path.startswith(("/", "\\")):
            raise ValueError(f"apiFiles[{index}].apiFilePath is invalid.")
        if ".." in Path(path).parts or not isinstance(item["payloadFiles"], dict):
            raise ValueError(f"apiFiles[{index}] contains invalid payload paths.")
        scanned_files.append(path)
    if scanned_files != sorted(set(scanned_files)):
        raise ValueError("OpenAPI workflow API paths must be sorted and unique.")

    specs = payload.get("specs")
    if not isinstance(specs, list):
        raise ValueError("OpenAPI generator response must contain a specs array.")

    required = {"domainApi", "openapi", "fileName", "serviceName", "sourcePath", "contentType"}
    ...
    for index, spec in enumerate(specs):
        ...
        openapi_document = spec["openapi"]
        if not isinstance(openapi_document, dict):
            raise ValueError(f"specs[{index}].openapi must be an object.")
        if openapi_document.get("openapi") != "3.1.0":
            raise ValueError(f"specs[{index}].openapi.openapi must be 3.1.0.")
        paths = openapi_document.get("paths")
        if not isinstance(paths, dict) or not paths:
            raise ValueError(f"specs[{index}].openapi.paths must be a non-empty object.")
        ...
        if source_path not in scanned_files:
            raise ValueError(
                f"specs[{index}].sourcePath must also appear in apiFiles."
            )
...
def main() -> None:
    ...
    openapi_agent_name = args.openapi_agent_name or get_agent_name(
        workflow, "openapi_specs_generator", "openapi-spec-workflow-hosted"
    )
    ...
    for repository in detector_output["repositories"]:
        ...
        print(
            f"Invoking OpenAPI workflow once for {repository_ref} "
            f"with source URL {source_url}."
        )
        generator_input = {"sourceDirectoryUrl": source_url}
        ...
        repo_output = {
            ...
            "scannedFiles": [
                item["apiFilePath"] for item in openapi_output["apiFiles"]
            ],
        ...
        if not openapi_output["specs"]:
            skip_output = {
                ...
                "repoURL": repository["repoURL"],
                "repository": repository_ref,
                "reason": "OpenAPI workflow returned no specs.",
            }
        ...
        for spec in openapi_output["specs"]:
            ...
            spec_path.write_text(
                json.dumps(spec["openapi"], indent=2) + "\n",
                encoding="utf-8",
            )
