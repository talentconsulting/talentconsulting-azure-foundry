...
[`repo-metadata-source-discovery`](agents/hosted/repometadata/repo-metadata-source-discovery/README.md) deterministically selects `.csproj`, `global.json`, `.bicep`, and `.tf` files, ignoring `bin`, `obj`, `packages`, `node_modules`, and `.terraform`. [`repo-metadata-generator`](agents/hosted/repometadata/repo-metadata-generator/README.md) deterministically parses each file's XML/JSON/source text directly -- no model is used -- into target framework, SDK version, and Azure resource entries. [`repo-metadata-workflow`](agents/hosted/repometadata/repo-metadata-workflow/README.md) writes one `<repository>/repo-metadata/repo-metadata.json` file through [`repo-metadata-pr-creator`](agents/hosted/repometadata/repo-metadata-pr-creator/README.md), while [`repo-metadata-manifest-orchestrator`](agents/hosted/repometadata/repo-metadata-manifest-orchestrator/README.md) processes only `repo-metadata` nodes and combines changed repositories with the manifest update in one PR. The catalog also includes a `lastCommitDate` field, the scanned path's most recent commit timestamp; this pipeline is intended to gather additional repository metadata over time, beyond just .NET versions.

The catalog's `azureResources` field extracts Azure resource `type`/`name` pairs (resource groups, storage accounts, SQL/Cosmos/Postgres/MySQL databases, etc.) by scanning `.bicep` `resource` declarations and Terraform `resource "azurerm_*" ...` blocks already committed to the repository -- never by querying live Azure. Only literal string resource names are resolved; a name built from a variable or string interpolation (e.g. `'${var}-thing'`) comes back as `name: null`. Only `.bicep` and `.tf` files are scanned -- `.bicepparam` files, raw ARM JSON templates, and Terraform `data` blocks are out of scope, and only Terraform resources whose type starts with `azurerm_` are included (other providers are skipped). **Operational note:** this pipeline's manifest entries set `repo-metadata.path-to-scan` to wherever the `.csproj`/`global.json` files live (often `src`), inherited from when this pipeline only looked for .NET version files. If a repository's Bicep/Terraform files live outside that scanned path (for example a top-level `infra/` or `infrastructure/` folder rather than under `src`), they will not be found until that repository's manifest entry's `path-to-scan` is widened to cover them -- this is a follow-up to check per-repository, not something the code can fix on its own.
