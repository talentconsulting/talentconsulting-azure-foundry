"""Responses host for manifest-driven local-dev-config generation."""
...
WORKFLOW_NAME = os.getenv("LOCAL_DEV_CONFIG_WORKFLOW_AGENT_NAME", "local-dev-config-workflow")
PUBLISHER_NAME = os.getenv("LOCAL_DEV_CONFIG_PR_CREATOR_AGENT_NAME", "local-dev-config-pr-creator")
MAX_ENTRIES = int(os.getenv("LOCAL_DEV_CONFIG_MANIFEST_MAX_ENTRIES", "25"))
MAX_CATALOGS = int(os.getenv("LOCAL_DEV_CONFIG_MANIFEST_MAX_CATALOGS", "100"))
...
async def handle_create(request: CreateResponse, context: ResponseContext, cancellation_signal: asyncio.Event):
    ...
    try:
        ...
        logger.exception("Manifest-driven local-dev-config orchestration failed.")
