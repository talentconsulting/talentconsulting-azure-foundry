"""Responses host for deterministic local-dev-config source discovery."""
...
async def handle_create(request: CreateResponse, context: ResponseContext, cancellation_signal: asyncio.Event):
    ...
    try:
        ...
        logger.exception("Local-dev-config source discovery failed.")
        result = {"localDevConfigFiles": [], "excludedFiles": []}
