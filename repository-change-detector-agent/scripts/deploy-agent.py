...
def find_existing_agent(project: AIProjectClient, name: str):
    # SDK versions differ slightly, so this keeps the lookup defensive.
    try:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Deploy an Azure AI Foundry agent from split source files.")
    parser.add_argument(
        "--agent-dir",
        default="agents/repository-change-detector",
        help="Path to the agent source directory."
    )
    parser.add_argument(
        "--project-endpoint",
    ...
    if existing_agent:
        # If your installed SDK exposes update_agent, this will update the existing agent in place.
        # If not, fall back to create-new-version mode.
        if hasattr(project.agents, "update_agent"):
