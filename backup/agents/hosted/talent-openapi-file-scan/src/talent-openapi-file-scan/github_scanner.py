...
class SourceFile:
    ...
    payload_files: tuple[tuple[str, tuple[str, ...]], ...] = ()
...
class GitHubClient:
    ...
    def get(self, api_path: str) -> dict[str, Any]:
        request = urllib.request.Request(
            ...
            headers={
                ...
                "User-Agent": "talent-openapi-file-scan",
    ...
    def raw_file(self, location: SourceLocation, path: str) -> str:
        ...
        request = urllib.request.Request(
            ...
            headers={"User-Agent": "talent-openapi-file-scan"},
        )
        try:
            with urllib.request.urlopen(request, timeout=60) as response:
        ...
        try:
            with zipfile.ZipFile(io.BytesIO(archive_bytes)) as archive:
                for member in archive.infolist():
                    ...
                    if separator and is_source_file(relative_path):
                        sources.append(
                            (
                                relative_path,
...
def is_source_file(path: str, base_path: str = "") -> bool:
    lowered = path.lower()
    ignored = {part.lower() for part in path.split("/")} & IGNORED_PARTS
    return lowered.endswith(".cs") and not ignored and _is_under_base(path, base_path)
...
def discover_action_type_names(content: str) -> set[str]:
    """Return type identifiers used by HTTP action parameters and return values."""
    names: set[str] = set()
    for match in ACTION_SIGNATURE_RE.finditer(content):
        if not HTTP_ATTRIBUTE_RE.search(match.group("attributes")):
            continue
        signature = (
            f'{match.group("attributes")} {match.group("return_type")} '
...
def attach_payload_files(
    controller: SourceFile,
    source_contents: dict[str, str],
) -> SourceFile:
    """Attach payload/model declarations referenced by a controller, transitively."""
    ...
    pending = list(discover_action_type_names(controller.content))
    visited_types: set[str] = set()
    scanned_paths: set[str] = set()
    payload_types_by_path: dict[str, set[str]] = {}
    while pending:
        ...
        for path in declarations.get(type_name, []):
            payload_types_by_path.setdefault(path, set()).add(type_name)
            if path in scanned_paths:
                continue
            scanned_paths.add(path)
            # Include nested payload/model types referenced by the definition.
            pending.extend(
                name
                for name in declarations
                if name not in visited_types
                and re.search(rf"\b{re.escape(name)}\b", source_contents[path])
            )
    ...
    return SourceFile(
        ...
        tuple(
            (path, tuple(sorted(payload_types_by_path[path])))
            for path in sorted(payload_types_by_path)
        ),
    )
...
def scan_source_url(source_url: str, client: GitHubClient | None = None) -> list[SourceFile]:
    ...
    source_contents = dict(github.archive_sources(location))
    files = []
    for path, content in source_contents.items():
        if not _is_under_base(path, location.base_path) or not is_controller(path):
            continue
        endpoints = discover_aspnet_endpoints(content)
        if endpoints:
            controller = SourceFile(path, content, endpoints)
            files.append(attach_payload_files(controller, source_contents))
...
def scan_inventory(
    source_url: str, client: GitHubClient | None = None
) -> dict[str, object]:
    sources = scan_source_url(source_url, client=client)
    return {
        "apiFiles": [
            {
                "apiFilePath": source.path,
                "payloadFiles": {
                    path: list(type_names)
                    for path, type_names in source.payload_files
                },
            }
