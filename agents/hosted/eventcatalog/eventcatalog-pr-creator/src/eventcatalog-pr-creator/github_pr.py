"""Validate and publish event and command catalogs through the GitHub API."""
...
class GitHubClient:
    ...
    def request(self, method: str, path: str, payload: object | None = None) -> Any:
        ...
        headers = {
            ...
            "User-Agent": "eventcatalog-pr-creator",
...
def validate_request(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    target_directory = _clean_relative_path(payload.get("targetDirectory", "event-catalog"), "targetDirectory")
    validated = []
    ...
    for item in catalogs:
        ...
        if not isinstance(catalog, dict) or set(catalog) != {"repository", "ref", "path", "commands", "events"}:
            raise PublicationError("invalid_catalog", "Each catalog must contain repository, ref, path, commands, and events.")
        if any(not isinstance(catalog[field], str) for field in ("repository", "ref", "path")) or not isinstance(catalog["commands"], list) or not isinstance(catalog["events"], list):
            raise PublicationError("invalid_catalog", "Each catalog contains invalid event or command fields.")
        output_path = _clean_relative_path(
            item.get("targetPath", posixpath.join(target_directory, _source_repository(item["sourceUrl"]), "events-and-commands.json")),
            "targetPath",
        )
    ...
    if total_bytes > MAX_TOTAL_BYTES:
        raise PublicationError("catalogs_too_large", "Generated event-and-command-catalog files exceed 10 MiB.")
...
def default_branch_name() -> str:
    ...
    return f"eventcatalog/{timestamp}-{uuid.uuid4().hex[:8]}"
...
def publish(payload: dict[str, Any], client: GitHubClient | None = None, branch_factory: Callable[[], str] = default_branch_name) -> dict[str, Any]:
    ...
    new_commit = github.request("POST", f"/repos/{repository}/git/commits", {
        "message": "Update generated event and command catalogs", "tree": tree["sha"], "parents": [base_sha]
    })
    ...
    pull_request = github.request("POST", f"/repos/{repository}/pulls", {
        "title": request.get("pullRequestTitle", "Update generated event and command catalogs"),
        "head": branch_name,
        "base": base_branch,
        "body": request.get("pullRequestBody", "Generated by the event-and-command-catalog workflow."),
    })
    return {
