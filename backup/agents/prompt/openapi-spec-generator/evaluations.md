...
## Evaluation 3: Payload source discovery

Given a controller and a `payloadFiles` mapping produced by `talent-openapi-file-scan`, the agent must read those files at the controller's repository ref and represent their supported fields in `requestBody`, response content, and `components.schemas`. It must not search for or read files absent from the mapping.
...
- The agent reads the controller and only source files listed in `payloadFiles`.
