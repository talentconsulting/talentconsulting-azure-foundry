...
- `content`: each spec `openapi`, serialized as formatted JSON when `openapi` is an object.
- `repository`: the top-level `repository`.
- `path`: the top-level `path`.

When serializing an `openapi` object, preserve the JSON structure and write it as pretty-printed JSON with a trailing newline. Do not convert it to YAML.
