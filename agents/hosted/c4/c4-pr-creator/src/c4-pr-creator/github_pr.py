"""Validate and publish c4 catalogs through the GitHub API."""
...
def _validate_c4_catalog(catalog: object) -> dict[str, Any]:
    if not isinstance(catalog, dict) or set(catalog) != {"repository", "ref", "path", "c4Model", "diagrams"}:
        raise PublicationError("invalid_catalog", "Each catalog must contain repository, ref, path, c4Model, and diagrams.")
    if any(not isinstance(catalog[field], str) for field in ("repository", "ref", "path")):
        raise PublicationError("invalid_catalog", "Each catalog contains invalid source fields.")
    if not isinstance(catalog["c4Model"], dict):
        raise PublicationError("invalid_catalog", "Each catalog contains an invalid c4Model.")
    diagrams = catalog["diagrams"]
    if not isinstance(diagrams, dict) or set(diagrams) != {"context", "container"}:
        raise PublicationError("invalid_catalog", "Each catalog must contain context and container diagrams.")
    for diagram_type, diagram in diagrams.items():
        if not isinstance(diagram, dict) or set(diagram) != {"format", "filename", "drawioXml"}:
            raise PublicationError("invalid_catalog", f"{diagram_type} diagram has an invalid shape.")
        if diagram["format"] != "drawio" or diagram["filename"] != f"{diagram_type}.drawio":
            raise PublicationError("invalid_catalog", f"{diagram_type} diagram must be drawio format.")
        if not isinstance(diagram["drawioXml"], str) or "<mxfile" not in diagram["drawioXml"]:
            raise PublicationError("invalid_catalog", f"{diagram_type} diagram must contain draw.io mxfile XML.")
    return catalog


def _catalog_files(catalog: dict[str, Any], directory: str) -> list[dict[str, bytes]]:
    return [
        {"path": posixpath.join(directory, "c4.json"), "content": (json.dumps(catalog, indent=2) + "\n").encode("utf-8")},
        {"path": posixpath.join(directory, "context.drawio"), "content": (catalog["diagrams"]["context"]["drawioXml"] + "\n").encode("utf-8")},
        {"path": posixpath.join(directory, "container.drawio"), "content": (catalog["diagrams"]["container"]["drawioXml"] + "\n").encode("utf-8")},
    ]
...
def validate_request(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    target_directory = _clean_relative_path(payload.get("targetDirectory", "c4"), "targetDirectory")
    validated = []
    ...
    for item in catalogs:
        if not isinstance(item, dict) or set(item) not in (
            ...
            {"sourceUrl", "catalog", "targetDirectory"}, {"sourceUrl", "catalog", "repository", "targetDirectory"},
        ):
            raise PublicationError(
                "invalid_catalog",
                "Each catalog must contain sourceUrl and catalog, with optional repository and targetDirectory.",
            )
        catalog = _validate_c4_catalog(item["catalog"])
        output_directory = _clean_relative_path(
            item.get("targetDirectory", posixpath.join(target_directory, _source_repository(item["sourceUrl"]), "c4")),
            "targetDirectory",
        )
        for output in _catalog_files(catalog, output_directory):
            if output["path"] in seen_paths:
                raise PublicationError("duplicate_output_path", f"Multiple catalogs map to {output['path']}.")
            seen_paths.add(output["path"])
            total_bytes += len(output["content"])
            validated.append({"sourceUrl": item["sourceUrl"], "path": output["path"], "content": output["content"]})
    ...
    if total_bytes > MAX_TOTAL_BYTES:
        raise PublicationError("catalogs_too_large", "Generated c4 files exceed 10 MiB.")
    result: dict[str, Any] = {
...
def default_branch_name() -> str:
    ...
    return f"c4/{timestamp}-{uuid.uuid4().hex[:8]}"
...
def publish(payload: dict[str, Any], client: GitHubClient | None = None, branch_factory: Callable[[], str] = default_branch_name) -> dict[str, Any]:
    ...
    new_commit = github.request("POST", f"/repos/{repository}/git/commits", {
        "message": "Update generated C4 catalogs", "tree": tree["sha"], "parents": [base_sha],
    })
    ...
    pull_request = github.request("POST", f"/repos/{repository}/pulls", {
        "title": request.get("pullRequestTitle", "Update generated C4 catalogs"),
        "head": branch_name,
        "base": base_branch,
        "body": request.get("pullRequestBody", "Generated by the c4 workflow."),
    })
    return {
