"""Responses-protocol host for the database-schema workflow."""
...
GENERATOR_NAME = os.getenv("DBSCHEMA_GENERATOR_AGENT_NAME", "dbschema-generator")
...
def error_response(error: Exception, source_url: str = "") -> dict[str, Any]:
    ...
    message = str(error) if isinstance(error, WorkflowError) else "The database-schema workflow could not be completed."
    return {
        "success": False,
