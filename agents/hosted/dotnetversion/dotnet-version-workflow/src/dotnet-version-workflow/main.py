"""Responses-protocol host for the dotnet-version workflow."""
...
DISCOVERY_NAME = os.getenv("DOTNET_VERSION_DISCOVERY_AGENT_NAME", "dotnet-version-source-discovery")
GENERATOR_NAME = os.getenv("DOTNET_VERSION_GENERATOR_AGENT_NAME", "dotnet-version-generator")
PR_CREATOR_NAME = os.getenv("DOTNET_VERSION_PR_CREATOR_AGENT_NAME", "dotnet-version-pr-creator")
DISCOVERY_MAX_FILES = int(os.getenv("DOTNET_VERSION_DISCOVERY_MAX_FILES", "100"))
GENERATOR_BATCH_SIZE = int(os.getenv("DOTNET_VERSION_GENERATOR_BATCH_SIZE", "5"))
...
def error_response(error: Exception, source_url: str = "") -> dict[str, Any]:
    ...
    message = str(error) if isinstance(error, WorkflowError) else "The dotnet-version workflow could not be completed."
    return {
        "success": False,
...
async def handle_create(request: CreateResponse, context: ResponseContext, cancellation_signal: asyncio.Event):
    ...
    try:
        ...
        logger.exception("dotnet-version workflow failed.")
