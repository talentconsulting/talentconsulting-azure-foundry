"""Responses-protocol host for the repo-metadata workflow."""
...
DISCOVERY_NAME = os.getenv("REPO_METADATA_DISCOVERY_AGENT_NAME", "repo-metadata-source-discovery")
GENERATOR_NAME = os.getenv("REPO_METADATA_GENERATOR_AGENT_NAME", "repo-metadata-generator")
PR_CREATOR_NAME = os.getenv("REPO_METADATA_PR_CREATOR_AGENT_NAME", "repo-metadata-pr-creator")
DISCOVERY_MAX_FILES = int(os.getenv("REPO_METADATA_DISCOVERY_MAX_FILES", "100"))
GENERATOR_BATCH_SIZE = int(os.getenv("REPO_METADATA_GENERATOR_BATCH_SIZE", "5"))
...
def error_response(error: Exception, source_url: str = "") -> dict[str, Any]:
    ...
    message = str(error) if isinstance(error, WorkflowError) else "The repo-metadata workflow could not be completed."
    return {
        "success": False,
...
async def handle_create(request: CreateResponse, context: ResponseContext, cancellation_signal: asyncio.Event):
    ...
    try:
        ...
        logger.exception("repo-metadata workflow failed.")
