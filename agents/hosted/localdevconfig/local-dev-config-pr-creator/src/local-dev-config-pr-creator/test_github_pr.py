...
def payload():
    return {
        ...
        "catalogs": [{
            ...
            "catalog": CATALOG,
            "targetPath": "app/local-dev-config/local-dev-config.json",
        }],
    }
...
class PublisherTests(unittest.TestCase):
    ...
    def test_validate_request_rejects_invalid_catalog_shape(self):
        ...
        with self.assertRaisesRegex(PublicationError, "repository, ref, path, localServices, and configurationKeys"):
    ...
    def test_validate_request_honors_target_directory_override(self):
        ...
        result = validate_request(request)
        self.assertEqual(
            "app/custom-dir/local-dev-config.json",
            result["catalogs"][0]["path"],
        )
    ...
    def test_publish_creates_one_commit_and_one_pull_request_when_content_differs(self):
        client = FakeClient()
        result = publish(payload(), client, branch_factory=lambda: "local-dev-config/test")
        self.assertEqual("created", result["status"])
        self.assertEqual(7, result["pullRequestNumber"])
        self.assertEqual("commit-sha", result["commitSha"])
        self.assertEqual("app/local-dev-config/local-dev-config.json", result["filesWritten"][0]["path"])
    ...
    def test_publish_includes_manifest_file_in_the_same_tree_as_catalogs(self):
        ...
        client = FakeClient()
        result = publish(request, client, branch_factory=lambda: "local-dev-config/manifest")
