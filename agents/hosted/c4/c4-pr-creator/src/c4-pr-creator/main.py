"""Responses-protocol host for c4 catalog publication."""
...
def error_response(error: Exception) -> dict[str, object]:
    ...
    message = str(error) if isinstance(error, PublicationError) else "The c4 catalogs could not be published."
    return {
        "success": False,
        "status": "failed",
        "repository": "",
...
async def handle_create(request: CreateResponse, context: ResponseContext, cancellation_signal: asyncio.Event):
    ...
    try:
        ...
        logger.exception("C4 publication failed.")
