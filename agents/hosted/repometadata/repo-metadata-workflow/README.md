...
Hosted Foundry agent that coordinates repository metadata discovery, generation, and optional publication for one repository.
...
A repository with no `.csproj`, `global.json`, `.bicep`, or `.tf` files is a valid, successful outcome: an empty `repoMetadataFiles` discovery result short-circuits the generator entirely and produces an empty catalog (`projects: []`, `sdks: []`, `azureResources: []`).

## Output

The response reports discovery/generation success, the merged repo-metadata catalog (including `azureResources`), publication status, and stable errors. Deferred results include `catalogs` and never create a pull request.
