"""Validate and publish .NET version catalogs through the GitHub API."""
...
class GitHubClient:
    ...
    def request(self, method: str, path: str, payload: object | None = None) -> Any:
        ...
        headers = {
            ...
            "User-Agent": "dotnet-version-pr-creator",
...
def validate_request(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    target_directory = _clean_relative_path(payload.get("targetDirectory", "dotnet-version"), "targetDirectory")
    validated = []
    ...
    for item in catalogs:
        ...
        if not isinstance(catalog, dict) or set(catalog) != {"repository", "ref", "path", "projects", "sdks"}:
            raise PublicationError("invalid_catalog", "Each catalog must contain repository, ref, path, projects, and sdks.")
        if (
            ...
            raise PublicationError("invalid_catalog", "Each catalog contains invalid dotnet-version fields.")
        output_path = _clean_relative_path(
            item.get(
                "targetPath",
                posixpath.join(_source_repository(item["sourceUrl"]), target_directory, "dotnet-version.json"),
            ),
            "targetPath",
        )
    ...
    if total_bytes > MAX_TOTAL_BYTES:
        raise PublicationError("catalogs_too_large", "Generated dotnet-version files exceed 10 MiB.")
...
def default_branch_name() -> str:
    ...
    return f"dotnet-version/{timestamp}-{uuid.uuid4().hex[:8]}"
...
def publish(payload: dict[str, Any], client: GitHubClient | None = None, branch_factory: Callable[[], str] = default_branch_name) -> dict[str, Any]:
    ...
    new_commit = github.request("POST", f"/repos/{repository}/git/commits", {
        "message": "Update generated .NET version catalog", "tree": tree["sha"], "parents": [base_sha]
    })
    ...
    pull_request = github.request("POST", f"/repos/{repository}/pulls", {
        "title": request.get("pullRequestTitle", "Update generated .NET version catalog"),
        "head": branch_name,
        "base": base_branch,
        "body": request.get("pullRequestBody", "Generated by the dotnet-version workflow."),
    })
    return {
