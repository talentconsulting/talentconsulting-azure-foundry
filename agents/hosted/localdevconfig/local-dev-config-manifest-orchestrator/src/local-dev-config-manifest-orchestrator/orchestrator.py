"""Manifest-driven local-dev-config workflow orchestration."""
...
MANIFEST_NODE = "local-dev-config"
...
def _read_url(url: str) -> bytes:
    headers = {"User-Agent": "local-dev-config-manifest-orchestrator"}
    if GITHUB_TOKEN:
...
def _validate_workflow_output(value: Any, entry: ManifestEntry) -> list[dict[str, Any]]:
    if not isinstance(value, dict) or not isinstance(value.get("success"), bool):
        raise ManifestError("invalid_workflow_output", "Local-dev-config workflow returned an invalid response.")
    if not value["success"]:
        errors = value.get("generationErrors") or value.get("errors") or []
        detail = errors[0].get("message") if errors and isinstance(errors[0], dict) else "Local-dev-config generation failed."
        raise ManifestError("local_dev_config_generation_failed", str(detail))
    catalogs = value.get("catalogs")
    if not isinstance(catalogs, list) or len(catalogs) != 1:
        raise ManifestError("invalid_workflow_output", "Local-dev-config workflow must return exactly one catalog.")
    item = catalogs[0]
    if not isinstance(item, dict) or set(item) != {"sourceUrl", "catalog"} or item["sourceUrl"] != entry.source_url:
        raise ManifestError("invalid_workflow_output", "Local-dev-config workflow returned an invalid catalog item.")
    catalog = item["catalog"]
    if not isinstance(catalog, dict) or set(catalog) != {"repository", "ref", "path", "localServices", "configurationKeys"}:
        raise ManifestError("invalid_workflow_output", "Local-dev-config workflow returned an invalid catalog.")
    if any(not isinstance(catalog[field], str) for field in ("repository", "ref", "path")):
        raise ManifestError("invalid_workflow_output", "Local-dev-config workflow returned an incomplete catalog.")
    if not isinstance(catalog["localServices"], list) or not isinstance(catalog["configurationKeys"], list):
        raise ManifestError("invalid_workflow_output", "Local-dev-config workflow returned invalid local services or configuration keys.")
    if (catalog["repository"], catalog["ref"], catalog["path"]) != (entry.repository_name, entry.ref, entry.scan_path):
        raise ManifestError("invalid_workflow_output", "Local-dev-config workflow returned mismatched source identity.")
    return [{
        "sourceUrl": item["sourceUrl"],
        "catalog": catalog,
        "targetPath": f"{entry.repository}/local-dev-config/local-dev-config.json",
    }]
...
def run_manifest(
    ...
    for entry, commit in changed:
        try:
            ...
            if len(combined_catalogs) + len(catalogs) > max_catalogs:
                raise ManifestError("too_many_catalogs", f"A run may publish at most {max_catalogs} local-dev-config catalogs.")
            combined_catalogs.extend(catalogs)
            updated_manifest[entry.index][MANIFEST_NODE]["last-commit-hash-scanned"] = commit
            warnings = [str(error.get("message", ""))[:300] for error in (workflow_result.get("generationErrors") or []) if isinstance(error, dict)]
            generated_repositories.append({"repository": entry.repository_name, "commit": commit, "warnings": warnings})
        except Exception as error:
            failures.append({
                "repository": entry.repository_name, "stage": "workflow",
                "errorType": type(error).__name__, "message": str(error)[:300],
            })
    ...
    publication = invoker(
        ...
        model,
        {
            ...
            "pullRequestTitle": "Update generated local development configuration",
            "pullRequestBody": (
                "Generated local development configuration for repositories whose branch-head commit changed. "
