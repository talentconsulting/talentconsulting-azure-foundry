...
Reads a shared public GitHub manifest, selects only `repo-metadata` nodes, compares branch-head commits, runs complete deferred repo-metadata workflows, and publishes generated target-framework and SDK-version catalogs plus updated commit hashes in one pull request. A repository whose workflow finds zero `.csproj`/`global.json` files is a valid, successful outcome.
