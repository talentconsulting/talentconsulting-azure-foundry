"""Manifest-driven dotnet-version workflow orchestration."""
...
# since this only reads the manifest itself from the same destination repository.
...
def _read_url(url: str) -> bytes:
    headers = {"User-Agent": "dotnet-version-manifest-orchestrator"}
    if GITHUB_TOKEN:
...
def _validate_workflow_output(value: Any, entry: ManifestEntry) -> list[dict[str, Any]]:
    if not isinstance(value, dict) or not isinstance(value.get("success"), bool):
        raise ManifestError("invalid_workflow_output", "dotnet-version workflow returned an invalid response.")
    if not value["success"]:
        errors = value.get("generationErrors") or value.get("errors") or []
        detail = errors[0].get("message") if errors and isinstance(errors[0], dict) else "dotnet-version generation failed."
        raise ManifestError("dotnet_version_generation_failed", str(detail))
    catalogs = value.get("catalogs")
    if not isinstance(catalogs, list) or len(catalogs) != 1:
        raise ManifestError("invalid_workflow_output", "dotnet-version workflow must return exactly one catalog.")
    item = catalogs[0]
    if not isinstance(item, dict) or set(item) != {"sourceUrl", "catalog"} or item["sourceUrl"] != entry.source_url:
        raise ManifestError("invalid_workflow_output", "dotnet-version workflow returned an invalid catalog item.")
    catalog = item["catalog"]
    if not isinstance(catalog, dict) or set(catalog) != {"repository", "ref", "path", "projects", "sdks"}:
        raise ManifestError("invalid_workflow_output", "dotnet-version workflow returned an invalid catalog.")
    if any(not isinstance(catalog[field], str) for field in ("repository", "ref", "path")):
        raise ManifestError("invalid_workflow_output", "dotnet-version workflow returned an incomplete catalog.")
    if not isinstance(catalog["projects"], list) or not isinstance(catalog["sdks"], list):
        raise ManifestError("invalid_workflow_output", "dotnet-version workflow returned invalid projects or sdks.")
    if (catalog["repository"], catalog["ref"], catalog["path"]) != (entry.repository_name, entry.ref, entry.scan_path):
        raise ManifestError("invalid_workflow_output", "dotnet-version workflow returned mismatched source identity.")
    return [{
        "sourceUrl": item["sourceUrl"],
        "catalog": catalog,
        "targetPath": f"{entry.repository}/dotnet-version/dotnet-version.json",
    }]
...
def run_manifest(
    ...
    for entry, commit in changed:
        try:
            ...
            if len(combined_catalogs) + len(catalogs) > max_catalogs:
                raise ManifestError("too_many_catalogs", f"A run may publish at most {max_catalogs} dotnet-version catalogs.")
    ...
    publication = invoker(
        ...
        model,
        {
            ...
            "pullRequestTitle": "Update generated .NET version catalog",
            "pullRequestBody": (
                "Generated .NET version catalog for repositories whose branch-head commit changed. "
